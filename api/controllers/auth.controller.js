...
export const signin = async (req, res, next) => {
    ...
    try {
        ...
        res
            .cookie('access_token', token, { httpOnly: true, expire: new Date(Date.now()) + 24 * 60 * 60 * 7 })
            .status(200)
            .json(rest);
...
export const google = async (req, res, next) => {
    ...
    try {
        ...
        if (user) {
            ...
            const newUser = new User({ username, firstname, lastname, email, password: hashedPassword, avatar });
            await newUser.save();
