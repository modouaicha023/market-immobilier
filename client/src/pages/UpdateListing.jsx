...
export default function UpdateListing() {
  ...
  const handleSumbit = async (e) => {
    ...
    try {
      ...
      if (+formData.regularPrice <= +formData.discountPrice)
        return setError("Discount price must be lower than the regumlar price");
  ...
  return (
    <main className="p-3 max-w-4xl mx-auto">
      ...
      <form className="flex flex-col sm:flex-row gap-4" onSubmit={handleSumbit}>
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          {error && <p className="text-red-700 text-s">{error}</p>}
        </div>
      </form>
    </main>
  );
